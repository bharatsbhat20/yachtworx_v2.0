...
export interface Database {
  public: {
    Tables: {
      ...
      marinas: {
        Row: MarinaRow;
        Insert: Omit<MarinaRow, 'created_at' | 'updated_at'>;
        Update: Partial<Omit<MarinaRow, 'id' | 'owner_id' | 'created_at' | 'updated_at'>>;
      };
      marina_berths: {
        Row: MarinaBerthRow;
        Insert: Omit<MarinaBerthRow, 'created_at' | 'updated_at'>;
        Update: Partial<Omit<MarinaBerthRow, 'id' | 'marina_id' | 'created_at' | 'updated_at'>>;
      };
      marina_berth_bookings: {
        Row: MarinaBerthBookingRow;
        Insert: Omit<MarinaBerthBookingRow, 'id' | 'created_at' | 'updated_at'>;
        Update: Partial<Omit<MarinaBerthBookingRow, 'id' | 'marina_id' | 'berth_id' | 'created_at' | 'updated_at'>>;
      };
      marina_staff: {
        Row: MarinaStaffRow;
        Insert: Omit<MarinaStaffRow, 'id' | 'created_at'>;
        Update: Partial<Omit<MarinaStaffRow, 'id' | 'marina_id' | 'user_id' | 'created_at'>>;
      };
      marina_provider_partnerships: {
        Row: MarinaProviderPartnershipRow;
        Insert: Omit<MarinaProviderPartnershipRow, 'id' | 'created_at' | 'updated_at'>;
        Update: Partial<Omit<MarinaProviderPartnershipRow, 'id' | 'marina_id' | 'provider_id' | 'created_at' | 'updated_at'>>;
      };
      marina_reviews: {
        Row: MarinaReviewRow;
        Insert: Omit<MarinaReviewRow, 'id' | 'created_at'>;
        Update: Partial<Omit<MarinaReviewRow, 'id' | 'marina_id' | 'reviewer_id' | 'created_at'>>;
      };
      marina_photos: {
        Row: MarinaPhotoRow;
        Insert: Omit<MarinaPhotoRow, 'id' | 'created_at'>;
        Update: Partial<Omit<MarinaPhotoRow, 'id' | 'marina_id' | 'created_at'>>;
      };
    };
    Views: {
      marina_analytics_view: {
        Row: MarinaAnalyticsViewRow;
      };
    };
...
// ─── Additional Module 5 row types ───────────────────────────────────────────

export interface MarinaPhotoRow {
  id: string;
  marina_id: string;
  storage_path: string;
...
/** Flat row returned by the marina_analytics_view (summary stats only). */
export interface MarinaAnalyticsViewRow {
  marina_id: string;
  marina_name: string;
  available_berths: number;
  total_berths: number;
...
// ─── Module 5 Insert helpers (camelCase → snake_case) ─────────────────────────

/**
 * Converts a camelCase MarinaBerthBooking partial into the snake_case row
 * shape expected by Supabase INSERT on marina_berth_bookings.
 */
export function marinaBerthBookingToInsert(
  b: Partial<MarinaBerthBooking>
): Omit<MarinaBerthBookingRow, 'id' | 'created_at' | 'updated_at'> {
  return {
    reference:            '',   // generated by DB trigger; leave blank
    marina_id:            b.marinaId ?? '',
    marina_name:          b.marinaName ?? '',
    berth_id:             b.berthId ?? '',
    berth_name:           b.berthName ?? '',
    boat_id:              b.boatId ?? null,
    boat_name:            b.boatName ?? null,
    owner_id:             b.ownerId ?? null,
    owner_name:           b.ownerName ?? null,
    booking_source:       b.bookingSource ?? 'online',
    ...
    nights:               b.nights ?? 1,
    rate_snapshot_usd:    b.rateSnapshotUsd ?? 0,
    total_amount_usd:     b.totalAmountUsd ?? 0,
    platform_fee_amount:  b.platformFeeAmount ?? 0,
    marina_payout_amount: b.marinaPayoutAmount ?? 0,
    status:               'CONFIRMED',
    special_requests:     b.specialRequests ?? null,
    payment_intent_id:    b.paymentIntentId ?? null,
    actual_check_in_at:   b.actualCheckInAt ?? null,
...
/**
 * Converts a booking status change + meta into the snake_case update shape
 * expected by Supabase UPDATE on marina_berth_bookings.
 */
export function marinaBerthBookingStatusToUpdate(
  status: MarinaBerthBooking['status'],
  now: string
): Partial<MarinaBerthBookingRow> {
  const update: Partial<MarinaBerthBookingRow> = {
    status,
    updated_at: now,
  };
  if (status === 'CHECKED_IN')  update.actual_check_in_at  = now;
  if (status === 'CHECKED_OUT') update.actual_check_out_at = now;
  if (status === 'CANCELLED')   update.cancelled_at        = now;
  return update;
}

/**
 * Converts a camelCase MarinaProviderPartnership partial into the snake_case
 * row shape expected by Supabase INSERT on marina_provider_partnerships.
 */
export function marinaPartnershipToInsert(
  p: Partial<MarinaProviderPartnership>
): Omit<MarinaProviderPartnershipRow, 'id' | 'created_at' | 'updated_at'> {
  return {
    marina_id:             p.marinaId ?? '',
    marina_name:           p.marinaName ?? null,
    provider_id:           p.providerId ?? '',
    provider_name:         p.providerName ?? null,
    provider_business_name: p.providerBusinessName ?? null,
    tier:                  p.tier ?? 'standard',
    status:                'pending',
    commission_rate:       p.tier === 'exclusive' ? 0.12 : p.tier === 'preferred' ? 0.08 : 0,
